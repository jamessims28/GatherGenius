...
export default function Page() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("gathergenius-experience-state");
      if (saved) {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("gathergenius-experience-state", JSON.stringify({ prompt, input, eventLock, status }));
    } catch {}
  ...
  async function buildFromPrompt(customPrompt = prompt) {
    ...
    setStatus(data.message || "Your experience is ready. One decision left.");
    setIsBuilding(false);
  }
        ...
        <section className="gg-hero">
          <div className="gg-inner">
            ...
            <motion.p className="gg-sub" initial={reducedMotion ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }}>
              One clean production app: zero-thinking UI, accessibility input, Apple-level motion, backend engine, Experience Lock™, and investor proof.
            </motion.p>
                ...
                <motion.div className="gg-panel" initial={reducedMotion ? false : { opacity: 0, y: 24, filter: "blur(12px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} transition={{ duration: 0.55 }}>
                  <div className="gg-card">
                    ...
                    <div className="gg-total">
                      ...
                      <motion.button className="gg-btn green" whileTap={reducedMotion ? {} : { scale: 0.94 }} onClick={lockEvent}>LOCK EXPERIENCE</motion.button>
                    </div>
                  </div>
