...
const globalTemplates = [
  "Wedding Reception","Wedding Ceremony","Engagement Party","Bridal Shower","Birthday Party","Kids Birthday","Baby Shower","Family Reunion","Backyard BBQ","Graduation Party","Corporate Conference","Seminar","Workshop","Product Launch","Networking Event","Trade Show","Corporate Retreat","Awards Gala","Company Holiday Party","Concert","Music Festival","Comedy Show","Open Mic","Club Night","Fashion Show","Gallery Opening","Town Hall","Political Rally","Campaign Fundraiser","Voter Registration Drive","Community Festival","Charity Fundraiser","Nonprofit Gala","Parade","Block Party","Sports Tournament","5K Run","Marathon","Fitness Bootcamp","Yoga Retreat","Esports Tournament","Food Festival","Wine Tasting","Cooking Class","Restaurant Opening","Food Truck Rally","Destination Retreat","Hotel Event","Travel Expo","Black Tie Gala","Luxury Brand Launch","Private Dinner","Red Carpet Event","VIP Lounge","Church Service Event","Baptism Reception","Eid Celebration","Diwali Celebration","Christmas Program","Halloween Party","Thanksgiving Dinner","Christmas Market","Hackathon","Startup Demo Day","Tech Meetup","AI Workshop","Health Fair","Wellness Retreat","Public Safety Fair","Military Ceremony","Veterans Event","Grand Opening","Pop-Up Shop","Vendor Market","Craft Fair"
];

const initialVendors = [
  { name: "Elite Sound DJs", category: "Music & Concert", service: "DJ / Music", city: "Stafford, VA", price: "$350 - $900", badge: "Featured", rating: "4.9", plan: "Featured" },
  { name: "Fresh Flame Catering", category: "Wedding", service: "Catering", city: "Fredericksburg, VA", price: "$15 - $45 / guest", badge: "Elite", rating: "4.8", plan: "Elite" },
  { name: "Venue Luxe Hall", category: "Corporate", service: "Venue", city: "Stafford, VA", price: "$800 - $5,000", badge: "Featured", rating: "4.9", plan: "Featured" },
  { name: "GlowPro Lighting", category: "Wedding", service: "Lighting / AV", city: "Washington, DC", price: "$300 - $1,500", badge: "Starter", rating: "4.7", plan: "Starter" },
  { name: "CivicStage Productions", category: "Political & Civic", service: "Stage / Civic Events", city: "Northern VA", price: "$1,500 - $7,500", badge: "Elite", rating: "4.6", plan: "Elite" },
  { name: "Premier Event Rentals", category: "Family & BBQ", service: "Tents / Tables", city: "Richmond, VA", price: "$500 - $2,500", badge: "Featured", rating: "4.8", plan: "Featured" }
];
...
const tractionStats = [
  ...
  { label: "Global Templates", value: "70+", note: "Included backend-connected templates" },
...
export default function Page() {
  ...
  const [systemStatus, setSystemStatus] = useState("Backend-connected package ready. Add keys for live mode.");
  ...
  async function loadVendors() {
    ...
    if (data.vendors?.length) {
      const mapped = data.vendors.map((vendor) => ({
  ...
  async function startCheckout(plan) {
    if (plan.name === "Starter") {
      setSystemStatus("Starter selected. No checkout needed.");
      return;
    }
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="card flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="orb" />
            <div>
              <p className="text-xs uppercase tracking-widest text-stone-500">Backend Connected Build</p>
        ...
        <section className="grid gap-6 lg:grid-cols-12">
          <div className="card lg:col-span-7">
            <p className="badge mb-4 inline-block">Backend endpoints connected with safe fallback</p>
            ...
            <p className="mt-5 max-w-2xl text-lg text-stone-600">
              Premium homepage, global templates, event builder, marketplace, vendor join, booking system, map/GPS, AI premium, Stripe checkout, Supabase routes, investor mode, outreach, and revenue projections.
            </p>
        ...
        {section === "home" && (
          <>
            <section className="grid gap-6 lg:grid-cols-3">
              <Panel title="Global Templates" subtitle="70+ included / 177+ platform target">
        ...
        {section === "vendors" && (
          <section className="grid gap-6 lg:grid-cols-2">
            ...
            <Panel title="Vendor Revenue Plans" subtitle="Stripe route connected">
              <div className="grid gap-3">
                {pricingPlans.slice(0, 3).map((plan) => <PlanCard key={plan.name} plan={plan} startCheckout={startCheckout} />)}
              </div>
            </Panel>
          </section>
        )}
        ...
        {section === "ai" && (
          <section className="grid gap-6 lg:grid-cols-2">
            <div className="card">
              ...
              <p className="mt-2 text-stone-500">OpenAI route connected with fallback mode.</p>
              <button onClick={generateAiPlan} className="btn primary mt-5">Generate Premium Plan</button>
              <button onClick={() => startCheckout({ name: "AI Premium" })} className="btn ml-2 mt-5">Subscribe $19/mo</button>
            </div>
            <div className="card">
        ...
        {section === "plans" && (
          <section>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              {pricingPlans.map((plan) => <PlanCard key={plan.name} plan={plan} startCheckout={startCheckout} />)}
            </div>
          </section>
        )}
        ...
        {section === "outreach" && (
          <section className="grid gap-6 lg:grid-cols-3">
            ...
            <Panel title="Outreach Status" subtitle="Backend-ready mode"><FeatureList items={["Manual workflow included", "Database routes ready", "Email automation can connect next", "Safe outreach copy included"]} /></Panel>
          </section>
        )}
...
function MapAndGrowth({ gpsStatus, detectGps, searchNearby, mapResults }) {
  return (
    <section className="grid gap-6 lg:grid-cols-2">
      <div className="card">
        <h3 className="mb-4 text-2xl font-light">Real Map + GPS</h3>
        ...
        <div className="mt-4 flex gap-2 flex-wrap">
          <button onClick={detectGps} className="btn primary">Detect GPS</button>
          <button onClick={searchNearby} className="btn">Search Nearby Vendors</button>
        </div>
        {mapResults?.length > 0 && (
          <div className="mt-4 grid gap-2">
            {mapResults.map((place) => <div key={place.name} className="rounded-2xl border bg-white p-3 text-sm">{place.name} — {place.address}</div>)}
          </div>
        )}
      </div>
      <div className="card">
...
function PlanCard({ plan, startCheckout }) {
  return (
    <div className="card">
      <p className="text-2xl font-light">{plan.name}</p>
      <p className="mt-2 text-3xl">{plan.price}</p>
      <FeatureList items={plan.details} />
      <button onClick={() => startCheckout(plan)} className="btn primary mt-4 w-full">{plan.name === "Starter" ? "Select Starter" : "Checkout"}</button>
    </div>
  );
}
