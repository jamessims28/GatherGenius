...
export default function Page() {
  ...
  const [systemStatus, setSystemStatus] = useState("Soft white automation build ready.");
  ...
  const nav = [
    ["home","Home"],["templates","Templates"],["builder","Builder"],["marketplace","Marketplace"],["vendors","Vendor Join"],["automation","Automation"],["map","Map + GPS"],["ai","AI Premium"],["plans","Pricing"],["investor","Investor"],["bookings","Bookings"],["outreach","Outreach"]
  ];

  return (
    <main className="min-h-screen p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="soft-card flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="orb" />
            <div>
              <p className="text-xs uppercase tracking-widest text-stone-500">Soft White Automation Build</p>
        ...
        <section className="grid gap-6 lg:grid-cols-12">
          <div className="soft-card lg:col-span-7">
            <p className="badge mb-4 inline-block">Soft white premium + automation</p>
            ...
            <p className="mt-5 max-w-2xl text-lg text-stone-600">
              Premium soft-white UI, global templates, marketplace, vendor join, booking system, AI planning, map/GPS, Stripe checkout, Supabase routes, outreach automation, vendor import, SEO generator, investor mode, and revenue projections.
            </p>
          ...
          <div className="soft-card bg-white lg:col-span-5">
            <p className="mb-4 text-sm text-stone-500">Transparent Scale Mode</p>
            <div className="grid grid-cols-2 gap-3">
              {tractionStats.slice(0,4).map((stat) => (
                <div key={stat.label} className="rounded-2xl bg-[#fbfaf7] border border-stone-200 p-4">
        ...
        {section === "automation" && (
          <section className="grid gap-6 lg:grid-cols-3">
            ...
            <Panel title="Outreach Automation" subtitle="Email route + manual safe mode"><FeatureList items={["Resend route included","DM script included","Follow-up workflow included","Logs appear below"]} /></Panel>
            ...
            <div className="card lg:col-span-3">
              <h3 className="text-2xl font-light">Outreach Log</h3>
              {outreachLog.length ? outreachLog.map((item, i) => <div className="mt-2 rounded-2xl border bg-white p-3 text-sm" key={i}>{item.vendor}: {item.message}</div>) : <p className="mt-3 text-stone-500">No outreach sent yet. Use vendor cards in Marketplace.</p>}
            </div>
          </section>
        )}
        ...
        {section === "bookings" && <section className="card"><h3 className="text-3xl font-light">Bookings Dashboard</h3>{bookings.length ? <div className="mt-4 grid gap-3 md:grid-cols-2">{bookings.map((booking) => <div key={booking.id} className="rounded-2xl border bg-white p-4"><p className="font-medium">{booking.id}</p><p className="text-sm text-stone-500">{booking.vendor} · {booking.status}</p><p className="mt-2 text-sm">Amount: {booking.amount} · Commission: {booking.commission}</p></div>)}</div> : <p className="mt-3 text-stone-500">No booking requests yet. Create one in Marketplace.</p>}</section>}

        {section === "outreach" && <section className="grid gap-6 lg:grid-cols-3"><Panel title="Vendor DM Script" subtitle="Manual outreach"><p className="text-stone-600">Hey — I launched GatherGenius to help people find vendors and send booking requests. I can list your business for free. Featured vendors get priority placement for $49/month. Want me to add you?</p></Panel><Panel title="Daily Workflow" subtitle="Acquisition routine"><FeatureList items={["Contact 20 vendors/day","Add 5 vendors/day","Pitch 1–2 Featured listings","Track status in spreadsheet/CRM"]} /></Panel><Panel title="Outreach Status" subtitle="Automation-ready mode"><FeatureList items={["Manual workflow included","Database routes ready","Email route included","SEO generator included"]} /></Panel></section>}
...
function Panel({title, subtitle, children}) { return <div className="card"><h3 className="text-2xl font-light">{title}</h3><p className="mb-5 text-stone-500">{subtitle}</p>{children}</div>; }
function Pill({children}) { return <div className="rounded-full border bg-white px-4 py-2 text-sm text-stone-700">{children}</div>; }
function FeatureList({items}) { return <div className="mt-3 grid gap-2">{items.map((item) => <div key={item} className="rounded-2xl border bg-white px-4 py-3 text-sm">✓ {item}</div>)}</div>; }
...
function Metric({label,value}) { return <div className="rounded-2xl border bg-white p-4"><p className="text-2xl font-light">{value}</p><p className="text-sm text-stone-500">{label}</p></div>; }
function PlanCard({plan,startCheckout}) { return <div className="card"><p className="text-2xl font-light">{plan.name}</p><p className="mt-2 text-3xl">{plan.price}</p><FeatureList items={plan.details} /><button onClick={() => startCheckout(plan)} className="btn primary mt-4 w-full">{plan.name === "Starter" ? "Select Starter" : "Checkout"}</button></div>; }
