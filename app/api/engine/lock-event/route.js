...
export async function POST(req) {
  ...
  const eventLock = body.eventLock || {};
  ...
  if (supabase) {
    await supabase.from("event_executions").insert({
      ...
      event_type: eventLock.intent?.eventType || "Event",
  ...
  if (!stripe) {
    return NextResponse.json({ ok: true, mode: "safe-preview", message: "Event lock prepared. Add STRIPE_SECRET_KEY for checkout.", checkout: { amount, lockCode: eventLock.lockCode } });
  }
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    line_items: [{
      price_data: {
        currency: "usd",
        product_data: { name: eventLock.name || "GatherGenius Event Lock" },
        unit_amount: Math.max(100, Math.round(amount * 100))
      },
      quantity: 1
    }],
    metadata: { lockCode: eventLock.lockCode || "", type: "event_lock_deposit" },
