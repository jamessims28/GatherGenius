...
import {
  ...
  CheckCircle2, Wallet, TrendingUp, Mail, Crown, Plus, ChevronDown, Settings,
  Hotel, MapPin, Send, RefreshCw
} from "lucide-react";
...
export default function Page() {
  ...
  const [status, setStatus] = useState("Production-ready app loaded.");
  ...
  const splitAmount = guests.length ? total / guests.length : 0;
  const collected = paidCount * splitAmount;
  const platformFeePercent = Number(process.env.NEXT_PUBLIC_PLATFORM_FEE_PERCENT || 10);
  const platformFee = collected * (platformFeePercent / 100);
  ...
  async function call(path, body) {
    try {
      ...
      setStatus("Request failed. Check deployment logs and environment variables.");
  ...
  async function runAiPlan() {
    const data = await call("/api/ai/plan", { items, guests, vendors, eventName, location, eventDate });
  ...
  async function searchPlaces(type) {
    const data = await call("/api/places/search", { type, location });
    setPlaces(data.results || []);
  }

  function exportItems() {
    const rows = [["Category","Item","Vendor","Cost","Status"], ...items.map(r => [r.category, r.item, r.vendor, r.cost, r.status])];
    const csv = rows.map(row => row.map(cell => `"${String(cell).replaceAll('"','""')}"`).join(",")).join("\\n");
    const blob = new Blob([csv], { type:"text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = "gathergenius-event-items.csv";
    a.click();
      ...
      <div className="flex min-h-screen">
        <aside className="hidden lg:flex w-72 shrink-0 flex-col border-r border-slate-200/80 bg-white/70 backdrop-blur-xl px-5 py-6">
          ...
          <div className="mt-auto rounded-3xl border border-slate-200 bg-white p-4 shadow-card">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-semibold">Production Build</p><p className="text-xs text-slate-500">{businessName}</p></div>
            </div>
          </div>
        </aside>
        ...
        <section className="flex-1">
          ...
          <div className="px-6 lg:px-10 py-8">
            ...
            {tab === "guests" && (
              <Panel title="Guests, RSVP, and Payments" icon={Users} subtitle="Invite guests, track RSVP, and collect split payments.">
                <div className="mb-5 grid md:grid-cols-4 gap-4">
                  ...
                  <MetricCard icon={Wallet} label="Split Amount" value={money(splitAmount)} />
                </div>
                <div className="grid gap-3">
                  {guests.map((g, idx) => (
                    <div key={g.name} className="grid lg:grid-cols-7 gap-3 items-center rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
                      <div><p className="font-semibold">{g.name}</p><p className="text-xs text-slate-500">{g.rsvp} · {g.paid ? "Paid" : "Unpaid"} · {g.reminderSent ? "Reminded" : "No reminder"}</p></div>
                      ...
                      <button onClick={() => call("/api/notifications/reminder", { guest:g, eventName, eventDate, location })} className="rounded-2xl bg-amber-50 text-amber-700 px-4 py-3 font-semibold">Remind</button>
                      <button onClick={() => checkout(Math.round(splitAmount * 100), "Guest Contribution", g.name)} className="rounded-2xl bg-indigo-600 text-white px-4 py-3 font-semibold">Pay</button>
                    </div>
                  ))}
                </div>
              </Panel>
            ...
            {tab === "places" && (
              <Panel title="Hotels and Venues" icon={Hotel} subtitle="Google Places-ready search scaffold.">
                <div className="flex flex-wrap gap-3 mb-5">
                  <button onClick={() => searchPlaces("hotels")} className="rounded-2xl bg-indigo-600 text-white px-5 py-3 font-semibold"><MapPin className="inline h-4 w-4 mr-2" />Find Hotels</button>
                  <button onClick={() => searchPlaces("event venues")} className="rounded-2xl bg-slate-100 text-slate-700 px-5 py-3 font-semibold">Find Venues</button>
                  <button onClick={() => searchPlaces("disc jockey")} className="rounded-2xl bg-slate-100 text-slate-700 px-5 py-3 font-semibold">Find DJs</button>
                </div>
                <div className="grid md:grid-cols-3 gap-4">{places.map((p, i) => <Card key={i}><h3 className="font-semibold">{p.name}</h3><p className="text-sm text-slate-500 mt-2">{p.address}</p><p className="text-xs text-indigo-600 mt-3">Rating: {p.rating || "N/A"}</p></Card>)}</div>
              </Panel>
            )}
            ...
            {tab === "settings" && (
              <Panel title="Settings" icon={Settings} subtitle="Production setup reminders.">
                <Card>
                  <div className="grid md:grid-cols-2 gap-4 text-sm text-slate-600">
                    <p><strong>Business Entity:</strong> {businessName}</p>
                    <p><strong>Project Name:</strong> gathergenius-lmh-enterprise</p>
                    <p><strong>Payments:</strong> Use LMH Enterprise LLC Stripe secret key.</p>
                    <p><strong>Database:</strong> Run docs/DATABASE_SCHEMA.sql in Supabase.</p>
                  </div>
                </Card>
              </Panel>
            )}
            ...
            {tab === "redeploy" && (
              <Panel title="Redeploy Guide" icon={Upload} subtitle="Deploy this production-ready calm version on Vercel.">
                <Card><ol className="list-decimal ml-5 space-y-3 text-slate-600"><li>Download and unzip this package.</li><li>Upload the unzipped folder to Vercel.</li><li>Project name must be lowercase: <code>gathergenius-lmh-enterprise</code>.</li><li>Add the environment variables from <code>.env.example</code>.</li><li>Redeploy the project.</li><li>Use LMH Enterprise LLC Stripe keys so payments route to the business.</li></ol></Card>
              </Panel>
            )}
