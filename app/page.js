...
import AmbientVoiceOS from "../components/AmbientVoiceOS";
...
export default function Page() {
  ...
  async function buildFromPrompt(customPrompt = prompt) {
    ...
    setIsBuilding(true);
    setEventLock(null);
    setStatus("Using background templates to build your experience...");
    setActiveStep(-1);
    ...
    try {
      ...
      setEventLock(lock);
      fetch("/api/evolution/run", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          transcript: cleanPrompt,
          confidence: lock?.confidenceScore || 94,
          total: lock?.total || 12800,
          intent: lock?.intent || {}
        })
      }).catch(() => null);
      setStatus(data.message || "GatherGenius produced your result.");
    } catch (error) {
      ...
      <main className="gg-shell">
        <motion.nav className="gg-nav" initial={reducedMotion ? false : { opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          ...
          <div className="gg-actions">
            ...
            <a className="gg-pill" href="/voice">Voice</a>
          </div>
        </motion.nav>
        ...
        <section className="gg-hero">
          <div className="gg-inner">
            ...
            <motion.h2 className="gg-title" initial={reducedMotion ? false : { opacity: 0, y: 18, filter: "blur(8px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }}>
              Speak once.
              <span>Then receive the result.</span>
            </motion.h2>

            <p className="gg-sub">Speak once. GatherGenius produces the result.</p>

            <AmbientVoiceOS onBuild={(voicePrompt) => {
              setPrompt(voicePrompt);
              buildFromPrompt(voicePrompt);
            }} />

            <div className="gg-minimal-command">
              <span className="gg-status good">Manual fallback</span>
              <input className="gg-input" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
              <button className="gg-btn secondary" onClick={() => buildFromPrompt()} disabled={isBuilding}>
                BUILD
              </button>
            </div>
