...
import {
  ...
  CreditCard, Building2, Send, DollarSign, Database, ShieldCheck
} from "lucide-react";
...
export default function Page() {
  ...
  const [status, setStatus] = useState("Layer 4 ready: Stripe + Supabase scaffolds added safely.");
  ...
  function requestBooking(vendor) {
    ...
    postJson("/api/bookings/create", { booking });
    setTab("bookings");
  }
      ...
      <div className="mx-auto max-w-7xl">
        <header className="card mb-6 flex flex-wrap justify-between gap-6 items-center">
          <div className="flex gap-5 items-center">
            <div className="orb" />
            <div>
              <p className="text-sm uppercase tracking-[.25em] text-stone-500">Layer 4 Stripe + Supabase</p>
              <h1 className="text-4xl md:text-5xl font-light tracking-wide">GatherGenius</h1>
              <p className="text-stone-500 mt-2">Marketplace, vendor system, Stripe routes, and Supabase storage by {businessName}</p>
            </div>
          </div>
          <button onClick={() => setTab("live")} className="btn primary">Live Setup</button>
        </header>
        ...
        {tab === "marketplace" && (
          <>
            <section className="card mb-6">
              ...
              <p className="text-stone-500 mt-4 max-w-2xl">Layer 4 adds real Stripe/Supabase route scaffolds. The app still works even before keys are added.</p>
            </section>
        ...
        {tab === "dashboard" && (
          <section>
            ...
            <div className="card">
              <h3 className="text-2xl font-light">Layer 4 is live</h3>
              <p className="text-stone-500 mt-2">Stripe and Supabase scaffolds are added without forcing keys during build.</p>
            </div>
          </section>
        )}
        ...
        {tab === "live" && (
          <section>
            <h2 className="text-3xl font-light mb-4 flex items-center gap-2"><ShieldCheck /> Live Setup</h2>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="card">
                <h3 className="text-2xl font-light mb-3">Supabase</h3>
                <p className="text-stone-500 mb-4">Run docs/database.sql, then add Supabase env vars in Vercel.</p>
                <p className="text-sm">NEXT_PUBLIC_SUPABASE_URL</p>
                <p className="text-sm">SUPABASE_SERVICE_ROLE_KEY</p>
              </div>
              <div className="card">
                <h3 className="text-2xl font-light mb-3">Stripe</h3>
                <p className="text-stone-500 mb-4">Create Featured and Elite monthly prices in Stripe, then add price IDs.</p>
                <p className="text-sm">STRIPE_SECRET_KEY</p>
                <p className="text-sm">STRIPE_VENDOR_PRICE_FEATURED</p>
                <p className="text-sm">STRIPE_VENDOR_PRICE_ELITE</p>
              </div>
            </div>
          </section>
        )}
