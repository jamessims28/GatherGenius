...
export async function getLivePricing(input = "") {
  ...
  const live = searchPricing?.prices?.length
    ? {
        ...fallback,
        mode: "live-pricing",
  ...
  return {
    ...
    nextBestAction:
      live.mode === "live-pricing"
        ? "confirm provider availability and lock estimate"
        : "connect live pricing APIs or confirm provider quote"
  };
}
