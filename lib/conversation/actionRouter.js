...
export function detectActionFromText(text = "") {
  ...
  if (t.includes("build") || t.includes("create") || t.includes("plan")) return "build_experience";
  if (t.includes("permission") || t.includes("access")) return "request_permission";
  if (t.includes("price") || t.includes("cost") || t.includes("budget")) return "estimate_price";
...
export function runLocalConversationAction({ text = "", currentLock = null } = {}) {
  ...
  if (action === "lock_experience") {
    if (!currentLock) {
      return {
        action,
        ok: false,
        message: "I can lock it after we build the experience first."
      };
    }
    ...
    return {
      action,
      ok: true,
      message: "I’m ready to lock the experience and open secure checkout."
    };
  }
  ...
  if (action === "request_permission") {
    return {
      action,
      ok: true,
      message: "I can ask for only the data I need and keep the rest private."
    };
  }
  ...
  if (action === "estimate_price") {
    return {
      action,
      ok: true,
      message: "I’ll price that using live pricing connectors when available, then fall back to a safe estimate if live data is not connected."
    };
  }
  ...
  return {
    action,
    ok: true,
    message: "I understand. I’ll keep this simple and help you move to the next best step."
  };
}
