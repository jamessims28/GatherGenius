...
export function buildEventLockFromPrompt(prompt) {
  ...
  const subtotal = primaryVendors.reduce((sum, v) => sum + Number(v.price || 0), 0);
  ...
  return {
    ...
    name: `${intent.eventType} — Guaranteed Event Outcome`,
    intent,
    budgetPlan,
    primaryVendors,
    backupVendors,
