  ...
  return (
    <main className="min-h-screen p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <header className="hero-card fade-in flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <div className="orb" />
            <div>
              <p className="text-xs uppercase tracking-widest text-stone-500">Elite Business Model Build</p>
              <h1 className="luxury-title text-5xl font-light">GatherGenius</h1>
              <p className="text-stone-500">AI event marketplace built for visibility, leads, and bookings</p>
        ...
        <section className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-9 space-y-6">
            <section id="overview" className="hero-card fade-in">
              <p className="badge mb-4 inline-block">Soft white elite platform</p>
              <h2 className="luxury-title text-6xl font-light leading-tight">Book your entire event in minutes.</h2>
              ...
              <div className="proof-strip mt-7">
                <div className="proof-pill">
                  <p className="text-xs uppercase tracking-widest text-stone-500">Trusted conversion path</p>
                  <p className="text-2xl font-light">AI Plan → Vendors → Booking</p>
                </div>
                <div className="proof-pill">
                  <p className="text-xs uppercase tracking-widest text-stone-500">Vendor value</p>
                  <p className="text-2xl font-light">Visibility + lead opportunities</p>
                </div>
                <div className="proof-pill">
                  <p className="text-xs uppercase tracking-widest text-stone-500">Launch target</p>
                  <p className="text-2xl font-light">First 50 vendors</p>
                </div>
              </div>
              <div className="mt-7 hero-visual"><div className="absolute bottom-6 left-6 right-6 rounded-3xl bg-white/75 backdrop-blur-xl border border-white/70 p-5 shadow-xl"><p className="text-xs uppercase tracking-widest text-stone-500">Premium launch experience</p><p className="luxury-title text-3xl">Events, vendors, AI planning, and growth automation in one platform.</p></div></div>
        ...
        {section === "overview" && (
          <>
            <section className="grid gap-6 lg:grid-cols-3 fade-in">
        ...
        {section === "templates" && (
          <section id="templates-section" className="card fade-in">
        ...
        {section === "vendors" && (
          <section className="grid gap-6 lg:grid-cols-2 fade-in">
            <div className="card">
        ...
        {section === "automation" && (
          <section id="automation-section" className="grid gap-6 lg:grid-cols-3 fade-in">
        ...
        {section === "ai" && (
          <section className="grid gap-6 lg:grid-cols-2 fade-in">
            <div className="card"><h3 className="luxury-title text-4xl font-light">AI Event Planner Premium</h3><p className="mt-2 text-stone-500">This is the monetization funnel: users create a plan, unlock premium output, then book vendors from the plan.</p><button onClick={generateAiPlan} className="btn primary mt-5">Generate Premium Plan</button><button onClick={() => startCheckout({name:"AI Premium"})} className="btn ml-2 mt-5">Subscribe $19/mo</button></div>
        ...
        {section === "investor" && (
          <section id="investor-section" className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-3">{tractionStats.map((stat) => <div key={stat.label} className="card"><p className="text-sm text-stone-500">{stat.label}</p><p className="mt-2 text-3xl font-light">{stat.value}</p><p className="mt-2 text-xs text-stone-400">{stat.note}</p><span className="modeled-label mt-3 inline-block">Modeled / projected</span></div>)}</div>
            <div className="grid gap-6 lg:grid-cols-2 fade-in"><Panel title="Live Activity Demo" subtitle="Simulated activity feed"><FeatureList items={activityFeed} /></Panel><Panel title="Revenue Projections" subtitle="Vendor acquisition model"><FeatureList items={["5 vendors/day = 150 vendors/month","20% Featured conversion target","Projected MRR: $1,470/mo","Annualized path: $17,640+","At 1,800 vendors/year: $88,200 annual MRR if all Featured"]} /></Panel></div>
          </section>
        )}

        {section === "bookings" && <section className="card"><h3 className="text-3xl font-light">Bookings Dashboard</h3>{bookings.length ? <div className="mt-4 grid gap-3 md:grid-cols-2">{bookings.map((booking) => <div key={booking.id} className="metric-tile"><p className="font-medium">{booking.id}</p><p className="text-sm text-stone-500">{booking.vendor} · {booking.status}</p><p className="mt-2 text-sm">Amount: {booking.amount} · Commission: {booking.commission}</p><p className="mt-2 text-xs text-stone-500">Vendor response target: within 24 hours · Booking ID confirmed</p></div>)}</div> : <p className="mt-3 text-stone-500">No booking requests yet. Create one in Marketplace.</p>}</section>}

        {section === "outreach" && <section className="grid gap-6 lg:grid-cols-3 fade-in"><Panel title="Vendor DM Script" subtitle="Manual outreach"><p className="text-stone-600">Hey — I launched GatherGenius to help people find vendors and send booking requests. I can list your business for free. Featured vendors get priority placement for $49/month. Want me to add you?</p></Panel><Panel title="Daily Workflow" subtitle="Acquisition routine"><FeatureList items={["Contact 20 vendors/day","Add 5 vendors/day","Pitch 1–2 Featured listings","Track status in spreadsheet/CRM"]} /></Panel><Panel title="Outreach Status" subtitle="Automation-ready mode"><FeatureList items={["Vendor email route included","Investor email route included","Outreach sequence generator","SEO generator included"]} /></Panel></section>}
...
function VendorCard({vendor, createBooking, sendOutreach, compact = false}) { return <div className="metric-tile"><div className="flex justify-between gap-3"><div><p className="font-medium">{vendor.name}</p><p className="text-sm text-stone-500">{vendor.service} · {vendor.city}</p></div><span className="h-fit rounded-full bg-stone-100 px-3 py-1 text-xs">{vendor.badge}</span></div>{!compact && <p className="mt-2 text-sm">{vendor.price} · Rating {vendor.rating}</p>}<div className="mt-3 flex flex-wrap gap-2"><span className="trust-signal">✓ Verified-ready</span><span className="trust-signal">Responds in 2 hrs</span><span className="trust-signal">Top local match</span></div><div className="mt-3 flex gap-2 flex-wrap"><button onClick={() => createBooking(vendor.name)} className="rounded-full bg-stone-900 px-4 py-2 text-sm text-white">Request Booking</button>{sendOutreach && <button onClick={() => sendOutreach(vendor)} className="rounded-full border bg-white px-4 py-2 text-sm">Send Outreach</button>}</div></div>; }
...
function PlanCard({plan,startCheckout}) { return <div className="card panel-border"><p className="text-2xl font-light">{plan.name}</p><p className="mt-2 text-3xl">{plan.price}</p><p className="mt-2 text-sm text-stone-500">{plan.name === "Featured" ? "Get more booking requests" : plan.name === "Elite" ? "Own premium placement and top visibility" : plan.name === "AI Premium" ? "Unlock full AI planning and vendor matches" : "Start free and test the platform"}</p><FeatureList items={plan.details} /><button onClick={() => startCheckout(plan)} className="btn primary mt-4 w-full">{plan.name === "Starter" ? "Select Starter" : "Checkout"}</button></div>; }
